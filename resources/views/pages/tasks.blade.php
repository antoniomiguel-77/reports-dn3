...
<section class="d-flex justify-content-center align-items-center flex-wrap col-md-12">
    ...
    <table class="table table-bordered  mt-4 text-center">
        ...
        <tbody>
                ...
                @foreach ($tasks as $item)
                    <tr class="{{($item->status === 'done')? ' alert alert-success text-decoration-line-through text-danger font-bold text-danger':''}} {{($item->status === 'pending')? ' alert alert-warning font-bold':''}}">
                        ...
                        <td>
                            ...
                            <a href="pendente/{{$item->id}}" class="btn btn-sm btn-outline btn-warning shadow-lg" title="Realizar mais tarde">
                                <i class="fa fa-pause"></i>
                            </a>
                            <a href="deletar/{{$item->id}}" class="btn btn-sm btn-outline btn-danger shadow-lg" title="Excluir tarefa">
            ...
            @else 
            <tr class="alert alert-info">
                <td colspan="5">Nenhum actividade</td>
            </tr>
            @endif
        </tbody>
    </table>
